...
class NewcalcmasterView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["closinglitdiesel"] = tank_master.objects.filter(date=datetime.utcnow().date()).aggregate(Sum('dieselbefore'))['dieselbefore__sum']
        ctx["openinglitdiesel"] = tank_master.objects.filter(date=datetime.utcnow().date()).aggregate(Sum('dieselafter'))['dieselafter__sum']
